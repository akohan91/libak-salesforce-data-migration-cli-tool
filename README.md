...
🔬 **Reference Analyzer & Dependency Config Generation** - Analyze your data structure to discover field-level relationships and auto-generate dependency configs (both insert and sync) before migration. Skips specified SObject dependencies and main tree record IDs.

🎭 **Flexible Reference Mapping** - Sync arbitrary references between orgs using master field matching (RecordTypes, PricebookEntries, Products, etc.). Supports conditional filtering and custom matching logic.
...
```json
   {
     "dependencies": {
       "dependencyConfigsToCreate": [],
       "dependencyConfigsToSync": [
         {
           "sObjectType": "RecordType",
           "masterField": "DeveloperName",
           "conditionField": null,
...
✅ Syncing references from dependencyConfigsToSync completed successfully
...
|----------|------|----------|-------------|
| `dependencies` | object | No | Configuration for managing reference mappings and prerequisite object migrations |
| `dependencies.dependencyConfigsToCreate` | array | No | Array of tree configurations for prerequisite objects that must be migrated before the main tree (e.g., Products for PricebookEntries) |
| `dependencies.dependencyConfigsToSync` | array | No | Array of reference mapping configurations for syncing records between orgs using master field matching (e.g., RecordTypes by DeveloperName) |
| `dependencies.dependencySobjectsToSkip` | array | No | Array of SObject API names to skip during reference analysis and dependency generation (e.g., ["User", "Group"]) |
...
#### Dependency Config To Sync Properties

| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `sObjectType` | string | Yes | The API name of the SObject to sync (e.g., "RecordType", "PricebookEntry") |
| `masterField` | string | Yes | The field name used for matching records between orgs (e.g., "DeveloperName" for RecordTypes) |
| `conditionField` | string | No | Field name to filter records (e.g., "SobjectType" for RecordTypes). Leave null/empty for no filtering |
| `conditionValues` | array | No | Array of values for the condition field. Leave null/empty for no filtering |

<blockquote><b>FLEXIBLE REFERENCE MAPPING:</b> The <code>dependencyConfigsToSync</code> allows you to sync any reference records between orgs using master field matching. Common use cases include:<br>
• <b>RecordTypes</b> - Match by DeveloperName, filter by SobjectType<br>
• <b>PricebookEntries</b> - Match by Product2.ProductCode<br>
• <b>Products</b> - Match by ProductCode<br>
• <b>Custom Objects</b> - Match by any unique external ID field<br><br>
RecordType syncing is included by default if not specified. The tool automatically detects all SObject types in your tree config and syncs their RecordTypes.</blockquote>
...
Analyze reference fields in your data structure and auto-generate both dependency insert configs (`dependencyConfigsToCreate`) and sync configs (`dependencyConfigsToSync`) for objects not included in the main "treeConfig" without performing migration:
...
- **Generates ready-to-use dependency configurations** in formatted JSON (both insert and sync configs)
...
3. **Dependencies Configs to Insert** - Complete insert configurations with:
   - SObject API names
   - Detected external ID fields
   - All referenced record IDs
4. **Dependencies Configs to Sync** - Reference mapping configurations with:
   - SObject API names
   - Empty masterField placeholders (to be filled manually)
   - Empty condition fields and values

This helps you understand your data structure and automatically generates both the `dependencyConfigsToCreate` and `dependencyConfigsToSync` arrays for complex migrations.
...
2. **Reference Syncing** - Processes `dependencyConfigsToSync` first, syncing reference records (RecordTypes, PricebookEntries, etc.) between orgs using master field matching
3. **Dependency Migration** - Processes `dependencyConfigsToCreate` array, migrating prerequisite objects in specified order
